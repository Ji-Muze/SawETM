...
class DeepConv1D(nn.Module):
    def __init__(self, nf, rf, nx):
        ...
        self.nf = nf
        if rf == 1:  # faster 1x1 conv
            w1 = torch.empty(nx, nf, dtype=torch.float32)
            nn.init.normal_(w1, std=0.02)
            self.w1 = Parameter(w1)
            self.b1 = Parameter(torch.zeros(nf, dtype=torch.float32))

            w2 = torch.empty(nf, nf, dtype=torch.float32)
            nn.init.normal_(w2, std=0.02)
            self.w2 = Parameter(w2)
            self.b2 = Parameter(torch.zeros(nf, dtype=torch.float32))
    ...
    def forward(self, x):
        if self.rf == 1:
            size_out = x.size()[:-1] + (self.nf,)
            x = torch.addmm(self.b1, x.float().view(-1, x.size(-1)), self.w1)
            rx = x
